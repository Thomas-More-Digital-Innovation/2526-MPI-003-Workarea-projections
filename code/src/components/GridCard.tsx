...
const GridCard = ({ id, title, description, active = false, onSelect }: CardProps) => {
    const handleClick = () => onSelect?.(id);

    return (
        <button type="button" onClick={handleClick} className={`w-[282px] h-[348px] rounded-2xl shadow-lg p-4 flex flex-col mt-4 text-left transition-colors ${active ? 'bg-[var(--color-secondary)] text-white' : 'bg-[var(--color-popup)] text-[var(--dark-text)]'}`}>
            {/* Image area */}
